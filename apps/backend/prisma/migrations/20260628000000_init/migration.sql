-- Migration: 20260628000000_init (Up)
-- Spec: 001-skeleton-bootstrap | FR-004, FR-005, FR-006
-- Stage 1 초기 마이그레이션: 8개 스키마 생성 + users 스키마 2개 테이블
--
-- GAP-001 안전망: Prisma ^6.19.0 multiSchema 에서 @@schema 모델이 없는 빈 7개 스키마
-- (products·commerce·orders·payments·settlements·admin·files)의 CREATE SCHEMA 가
-- 자동 생성되지 않을 수 있어 8개 전부 IF NOT EXISTS(멱등)로 명시.
...
--    FR-005: 토큰값(해시)·만료일시·무효화 여부 포함
--    ADR-003: tokenHash = sha256(원문 JWT). 원문 token 컬럼 없음.
--             SHA-256 채택: refresh 검증은 원문으로 DB row 를 lookup 해야 하므로
--             salt 가 매번 달라지는 bcrypt 는 동등 비교 조회 불가.
--             고엔트로피 JWT(jti uuid 포함)라 무염 SHA-256 의 위험이 낮다.
...
-- jti(uuid) 포함 JWT 이므로 해시 충돌 없이 UNIQUE 적용 가능.
